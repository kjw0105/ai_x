...
function getSeverityKorean(severity: string): string {
  const map: Record<string, string> = { error: "?¨Í∞Å", warn: "Í≤ΩÍ≥†", info: "?ïÎ≥¥" };
...
function parseTBMSummary(summary: string): { title: string; content: string }[] {
  ...
  for (const line of lines) {
    ...
    // Match section headers like "1) ?§Îäò ?ëÏóÖ Í∞úÏöî" or "1. ?§Îäò ?ëÏóÖ Í∞úÏöî" or "**1) ?§Îäò ?ëÏóÖ Í∞úÏöî**"
...
<body>
  <div class="header">
    <h1>?àÏ†Ñ ?êÍ? Î≥¥Í≥†??/h1>
    <div class="subtitle">?§Îßà???àÏ†ÑÏßÄ?¥Ïù¥ - Í≤ΩÏÉÅ?®ÎèÑ Ï§ëÏÜåÍ∏∞ÏóÖ ÏßÄ???úÏä§??/div>
  </div>

  <div class="info-box">
    <div class="info-row">
      <div class="info-label">?åÏùºÎ™?/div>
      <div class="info-value">${escapeHtml(data.fileName)}</div>
    </div>
    ${data.projectName ? `
    <div class="info-row">
      <div class="info-label">?ÑÎ°ú?ùÌä∏</div>
      <div class="info-value">${escapeHtml(data.projectName)}</div>
    </div>` : ""}
    ${data.documentType ? `
    <div class="info-row">
      <div class="info-label">Î¨∏ÏÑú ?†Ìòï</div>
      <div class="info-value">${escapeHtml(data.documentType)}</div>
    </div>` : ""}
    <div class="info-row">
      <div class="info-label">?ùÏÑ± ?†Ïßú</div>
  ...
  <div class="section">
    <div class="section-title">TBM (?ëÏóÖ ???Ä?? ?îÏïΩ</div>
    ${
      tbmSummary.length > 0
        ? (() => {
        ...
        : `<div class="muted">TBM ?îÏïΩ???ÜÏäµ?àÎã§.</div>`
    }
    ...
    <div style="margin-top:25px;page-break-before:avoid;">
      <div style="font-weight:bold;color:#475569;margin-bottom:10px;font-size:15px;display:flex;align-items:center;gap:8px;">
        <span style="display:inline-flex;align-items:center;justify-content:center;width:28px;height:28px;border-radius:50%;background:#3b82f6;color:white;font-size:18px;">?ìù</span>
        ?ÑÏÇ¨ ?¥Ïö© (Transcript)
      </div>
  ...
  <div class="section">
    <div class="section-title">Í≤ÄÏ¶??îÏïΩ</div>
    <div class="summary-grid">
      <div class="summary-card">
        <div class="summary-label">Ï¥?Î¨∏Ï†ú??/div>
        <div class="summary-value">${data.summary.totalIssues}</div>
      </div>
      <div class="summary-card">
        <div class="summary-label">?¨Í∞Å??Î¨∏Ï†ú</div>
      ...
      <div class="summary-card">
        <div class="summary-label">?ïÎ≥¥</div>
  ...
  <div class="section">
    <div class="section-title">Î∞úÍ≤¨??Î¨∏Ï†ú??/div>
    ${issues.length === 0 ? `
      <div class="no-issues">??Î∞úÍ≤¨??Î¨∏Ï†úÍ∞Ä ?ÜÏäµ?àÎã§. Î™®Îì† Í≤ÄÏ¶ùÏùÑ ?µÍ≥º?àÏäµ?àÎã§!</div>
    ` : `
      <table class="issues-table">
        <thead>
          <tr>
            <th style="width:50px;">#</th>
            <th style="width:100px;">?¨Í∞Å??/th>
            <th>Î¨∏Ï†ú ?¥Ïö©</th>
          </tr>
        </thead>
        <tbody>
  ...
  <div class="footer">
    <div>Generated by Smart Safety Guardian (?§Îßà???àÏ†ÑÏßÄ?¥Ïù¥)</div>
...
export async function POST(req: Request) {
  try {
    ...
    if (data.tbmSummary && data.tbmSummary.length > 0) {
      ...
      logger.log("[API Export PDF] ?†Ô∏è TBM Summary is EMPTY or missing");
    }
    ...
    if (datePattern.test(data.fileName)) {
      const cleanFileName = data.fileName.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9Í∞Ä-??-]/g, "_");
