...
export async function exportReportToPDF(data: ExportData) {
        ...
        <body>
            ...
            <div class="section">
                ...
                ${data.issues.length === 0 ? `
                    <div class="no-issues">
                        ??Î∞úÍ≤¨??Î¨∏Ï†úÍ∞Ä ?ÜÏäµ?àÎã§. Î™®Îì† Í≤ÄÏ¶ùÏùÑ ?µÍ≥º?àÏäµ?àÎã§!
                    </div>
                ` : `
    ...
    loadingMsg.textContent = 'PDF ?ùÏÑ± Ï§?.. ?†ÏãúÎß?Í∏∞Îã§?§Ï£º?∏Ïöî';
    ...
    try {
        ...
        if (!pdfBlob) {
            loadingMsg.textContent = '??PDF ?ùÏÑ± ?ÑÎ£å';
        ...
        loadingMsg.textContent = `??PDF ?ùÏÑ± ?ÑÎ£å! (${Math.round(pdfBlob.size / 1024)}KB)`;
        loadingMsg.style.color = '#22c55e';
        ...
        loadingMsg.textContent = '??PDF ?ùÏÑ± ?§Ìå®';
